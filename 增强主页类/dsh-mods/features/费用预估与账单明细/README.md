...
- **每轮费用柱状图**：`CostTurnChart` 竖排柱状图（柱高 ∝ 费用，颜色按模型区分，柱顶显示金额、柱底显示轮次 `#N`，悬停显示 模型·步数·金额），替换原横向长条；柱宽 34px、柱间间隔 = 柱宽 1/3，每页柱子数按容器宽度自适应（铺满约 85–90% 宽度、左右留白、整体居中），分页条与柱组对齐居中；高度 150px，柱高采用**对数刻度**（锚定最小值：2% + 98%·ln(v/min)÷ln(max/min)），费用集中时也能拉开明显高度差
