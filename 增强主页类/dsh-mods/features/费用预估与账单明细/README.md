...
- **每轮费用柱状图**：`CostTurnChart` 竖排柱状图（柱高 ∝ 费用，颜色按模型区分，柱顶显示金额、柱底显示轮次 `#N`，悬停显示 模型·步数·金额），替换原横向长条；柱宽 34px、柱间间隔 = 柱宽 1/3，柱组居中，分页条与柱组对齐居中
- **每轮费用分页**：超过 10 轮自动分页（上一页 / 第 x / y 页 / 下一页，按钮仅在有分页时显示）
