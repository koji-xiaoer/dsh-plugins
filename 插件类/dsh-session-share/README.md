...
| 消息内胶囊芯片 | `patches/dsh-client-ui-conversation-sharechip.patch`:官方 `projectUserText` 增加 `shr-*` token 识别,消息气泡内渲染胶囊芯片并显示 **`会话:标题前10字…`**(经 `/sshp/resolve` 异步解析,悬停显示 ID+全标题;解析失败/未达时回退纯 ID) |
